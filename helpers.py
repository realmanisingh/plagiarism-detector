...
def containment(ngram_array):
    ...
    # Getting the answer and source texts
    answer = ngram_array[0]
    source = ngram_array[1]
    
    # Iterating through the answer to find the length of the intersection
    i = 0
    intersect = 0
    while i < len(answer):
        if answer[i] == source[i]:
            intersect += 1
        i += 1
    print(intersect)
    
    # Getting the total number of ngrams (denominator)
    total_ngram = len(answer[answer==1])
    
    # Calculate and return the containment
    return intersect / total_ngram